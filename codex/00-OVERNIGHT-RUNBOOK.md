...
Advance **bumpbox** without breaking deterministic guarantees or repo coherence.

## First pass

1. Run `cargo test --workspace --all-features` and `python3 scripts/validate_contract_fixtures.py` to establish the baseline.
2. Read `docs/02-TECHNICAL-ARCHITECTURE.md` and the current query tests before changing math or collision semantics.
3. Choose the highest-priority non-done item from `codex/taskboard.yaml`.
4. Write a failing test first, then implement the minimal deterministic change.
